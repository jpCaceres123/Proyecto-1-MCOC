...
Este documento explica cómo se construyó el modelo 3D del edificio de Ingeniería en la rama `Edificio-1`. La fuente de geometría es `geometria_manual.json`. A partir de esa fuente se generan los nodos, elementos, muros, losas, archivos para OpenSeesPy y el CSV utilizado por Unity.
...
1. Editar `geometria_manual.json`.
2. Ejecutar `generar_modelo_manual.py`.
3. El generador actualiza:
   - `modelo_3d_manual.json`.
   - `nodos_modelo_manual.xlsx`.
   - `UnityVisualization/Assets/Resources/model_3d.csv`.
4. Ejecutar `modelo_opensees_3d.py` para construir el modelo de OpenSeesPy.
...
Los muros se guardan en la propiedad `walls` de `geometria_manual.json`. Se exportan al CSV y Unity los dibuja como sólidos verticales.
...
La geometría de las losas se guarda en `modelo_3d_manual.json` y en la hoja `Losas` del Excel.

## 10. OpenSeesPy

Las losas se modelan en `modelo_opensees_3d.py` mediante:
...
Se deben regenerar desde `geometria_manual.json`.
...
```powershell
.venv\Scripts\python.exe generar_modelo_manual.py
```

Para verificar que OpenSeesPy pueda construirlo:

```powershell
.venv\Scripts\python.exe modelo_opensees_3d.py
```
