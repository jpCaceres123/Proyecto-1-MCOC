...
def build_model():
    ...
    if data.get("beam_slab_loads"):
        apply_slab_loads_to_beams(data)
    return data


def apply_slab_loads_to_beams(data):
    """Apply slab self-weight resultants to existing beam end nodes.

    The generated data keeps the real tributary distribution shape for each beam
    (uniform, triangular, or trapezoidal). OpenSees element loads for the current
    elasticBeamColumn setup are not used here because many tributary loads are
    non-uniform; this applies their vertical resultants conservatively without
    changing beam or node connectivity.
    """
    nodes_by_element = {element["id"]: (element["i"], element["j"])
                        for element in data["elements"]}
    nodal_loads = {}
    for slab_load in data["beam_slab_loads"]:
        beam_nodes = nodes_by_element.get(slab_load["beam_id"])
        if not beam_nodes:
            continue
        vertical_load = -slab_load["total_load_kN"]
        for node_id in beam_nodes:
            nodal_loads[node_id] = nodal_loads.get(node_id, 0.0) + vertical_load / 2.0

    ops.timeSeries("Linear", 1)
    ops.pattern("Plain", 1, 1)
    for node_id, pz in nodal_loads.items():
