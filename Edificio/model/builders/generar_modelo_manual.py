...
def split_walls_by_story(walls, levels):
    """Return one analytical wall panel per storey, retaining source identity."""
    result = []
    for wall in walls:
        ...
        if not cuts or abs(cuts[0] - lo) > 1e-8 or abs(cuts[-1] - hi) > 1e-8:
            raise ValueError(f"Muro {wall['id']}: extremos no coinciden con niveles")
        for z0, z1 in zip(cuts, cuts[1:]):
            floor = next(i for i, z in enumerate(levels[1:], 1) if abs(float(z) - z1) < 1e-8)
            panel = dict(wall)
            panel.update(id=int(wall["id"]) * 100 + floor,
                         source_wall_id=int(wall["id"]), floor=floor,
                         source_z_i_m=lo, source_z_j_m=hi,
                         z_i_m=z0, z_j_m=z1,
                         status="STRUCTURAL_WALL_STORY")
            result.append(panel)
    if len({wall["id"] for wall in result}) != len(result):
...
def main():
    ...
    # Los extremos deben coincidir con nodos de viga ya declarados; si falta
    # uno, ensure_beam_node lo crea en la coordenada fuente exacta.
    for steel_column in config.get("steel_columns", []):
        a = ensure_beam_node(steel_column["x_m"], steel_column["y_m"], steel_column["z_i_m"])
        b = ensure_beam_node(steel_column["x_m"], steel_column["y_m"], steel_column["z_j_m"])
        elements.append({"id": eid, "type": "STEEL_COLUMN_SHS300x20",
                         "i": a["id"], "j": b["id"], "direction": "Z",
                         "axis": steel_column["axis"], "member_id": steel_column["id"],
                         "status": "STRUCTURAL_MANUAL"})
        eid += 1
    ...
    # Las vigas manuales terminan en la cara de algunas columnas (por ejemplo,
    # E' en x=-0.25 m frente al eje E en x=0.00 m). Sin este tramo corto el
    # modelo deja libre el desplazamiento vertical del extremo, aunque en la
    # geometria fisica la viga llega a la columna. Se agregan los enlaces luego
    # de calcular las areas tributarias para que no creen paneles de losa.
    node_by_id = {node["id"]: node for node in nodes}
    column_node_ids = {
        node_id
        for element in elements
        if element["type"] in ("COLUMN", "STEEL_COLUMN_SHS300x20")
        for node_id in (element["i"], element["j"])
    }
    beam_node_ids = {
        node_id
        for element in elements
        if element["type"].startswith("BEAM")
        for node_id in (element["i"], element["j"])
    }
    link_limit = float(config.get("frame_face_link_max_m", 0.0))
    frame_face_links = []
    next_link_id = max(element["id"] for element in elements) + 1
    for beam_node_id in sorted(beam_node_ids - column_node_ids):
        beam_node = node_by_id[beam_node_id]
        candidates = []
        for column_node_id in column_node_ids:
            column_node = node_by_id[column_node_id]
            if abs(beam_node["z_m"] - column_node["z_m"]) > 1e-6:
                continue
            dx = column_node["x_m"] - beam_node["x_m"]
            dy = column_node["y_m"] - beam_node["y_m"]
            distance = (dx * dx + dy * dy) ** 0.5
            if 1e-6 < distance <= link_limit + 1e-9:
                candidates.append((distance, column_node_id, dx, dy))
        if not candidates:
            continue
        distance, column_node_id, dx, dy = min(candidates)
        direction = "X" if abs(dx) >= abs(dy) else "Y"
        elements.append({
            "id": next_link_id,
            "type": "BEAM_RIGID_OFFSET",
            "direction": direction,
            "i": column_node_id,
            "j": beam_node_id,
            "status": "STRUCTURAL_FACE_TO_AXIS_LINK",
        })
        frame_face_links.append({
            "element_id": next_link_id,
            "column_node": column_node_id,
            "beam_node": beam_node_id,
    ...
    steel_section = config.get("steel_column_section", {})
    steel_b = steel_section.get("outer_width_m", 0.30)
    steel_t = steel_section.get("wall_thickness_m", 0.020)
    steel_inner = steel_b - 2.0 * steel_t
    if steel_inner <= 0.0:
        raise ValueError("Espesor invalido para columna tubular de acero")
    steel_area = steel_b ** 2 - steel_inner ** 2
    ...
    for load in wall_load_cases:
        ...
        if len(candidates) != 1:
            raise ValueError(f"Carga en muro origen {source_id}, z={level}: paño inferior ambiguo")
    ...
    data = {"source": "geometria_manual.json", "load_source": "cargas_losas.json", "status": "MANUAL_REVIEW",
             ...
             "lt2_load_zones": loads.get("lt2", {}),
              ...
              "steel_columns": config.get("steel_columns", []),
              "frame_face_links": frame_face_links,
            ...
            "section_steel_columns": {"shape": steel_section.get("type", "SHS_300x300x20"),
                                      "outer_width_m": steel_b, "wall_thickness_m": steel_t,
                                      "A_m2": steel_area, "Iy_m4": steel_inertia,
    ...
    for wall in wall_segments:
        lines.append(
            ...
            f'{wall["x_j_m"]},{wall["y_j_m"]},{wall["z_j_m"]},{wall["thickness_m"]},{wall.get("status", "MANUAL")};origen={wall["source_wall_id"]};piso={wall["floor"]}'
        )
    for slab in slabs:
