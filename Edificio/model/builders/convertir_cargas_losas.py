...
SOURCE = ROOT / "data" / "loads" / "Cargas_losas.txt"
OUTPUT = ROOT / "data" / "loads" / "cargas_losas.json"
...
def main():
    ...
    # Normalizacion de la geometria final de semana-4-final. El TXT historico
    # conserva vertices de levantamientos anteriores; el JSON final define la
    # geometria oficial usada por el modelo y sus verificaciones.
    def geometry_by_type(zone, kind):
        return next((item for item in zone["geometries"] if item["type"] == kind), None)

    for level in levels:
        if level["z_m"] in (3.96, 15.84):
            zone_2 = next(item for item in level["zones"] if item["id"] == "Zona 2")
            polygon = geometry_by_type(zone_2, "polygon")
            if level["z_m"] == 3.96:
                polygon["points"] = [
                    [-0.25, 7.25], [10.25, 7.25], [10.25, 10.15],
                    [3.60, 10.15], [3.60, 12.15], [-0.25, 12.15]]
            else:
                polygon["points"] = [
                    [-0.25, 7.25], [10.25, 7.25], [10.25, 10.15],
                    [3.60, 10.15], [3.60, 12.15], [-0.25, 12.15]]
            if level["z_m"] == 3.96:
                zone_3 = next(item for item in level["zones"] if item["id"] == "Zona 3")
                geometry_by_type(zone_3, "polygon")["points"] = [
                    [-0.25, 12.15], [6.30, 12.15], [6.30, 10.15],
                    [10.25, 10.15], [10.25, 16.50], [-0.25, 16.50]]
        if level["z_m"] in (7.92, 11.88):
            zone = next(item for item in level["zones"] if item["id"] == "Zona 3")
            polygon = geometry_by_type(zone, "polygon")
            polygon["points"] = [
                [-0.25, 12.15], [6.3, 12.15], [6.3, 10.0],
                [45.3, 10.0], [45.3, 16.5], [-0.25, 16.5]]
            void = geometry_by_type(zone, "void")
            void["points"] = [[3.6, 10.0], [6.4, 10.0],
                               [6.4, 12.25], [3.6, 12.25]]
        elif level["z_m"] == 15.84:
            zone_1 = next(item for item in level["zones"] if item["id"] == "Zona 1")
            zone_2 = next(item for item in level["zones"] if item["id"] == "Zona 2")
            zone_3 = next(item for item in level["zones"] if item["id"] == "Zona 3")
            stair = next((item for item in zone_2["geometries"]
                          if item.get("id") == "Vacío de escalera:"), None)
            if stair is not None:
                zone_2["geometries"].remove(stair)
                zone_1["geometries"].append(stair)
            polygon = geometry_by_type(zone_3, "polygon")
            polygon["points"] = [
                [-0.25, 12.15], [6.3, 12.15], [6.3, 10.15],
                [10.25, 10.15], [10.25, -0.25], [19.7, -0.25],
                [19.7, -4.42], [30.3, -4.42], [30.3, -0.25],
                [50.3, -0.25], [50.3, 16.5], [-0.25, 16.5]]
            void = geometry_by_type(zone_3, "void")
            void["points"] = [[3.6, 10.15], [6.3, 10.15],
                               [6.3, 12.15], [3.6, 12.15]]
