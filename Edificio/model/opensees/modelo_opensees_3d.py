...
def build_model():
    ...
    for element in data["elements"]:
        ...
        if element["type"] == "STEEL_COLUMN_SHS300x20":
            section = data["section_steel_columns"]
            material = data["material_steel"]
        ...
        E = material["E_kPa"] if element["type"] == "STEEL_COLUMN_SHS300x20" else data["material"]["E_kPa"]
        nu = material["nu"] if element["type"] == "STEEL_COLUMN_SHS300x20" else data["material"]["nu"]
        G = E / (2.0 * (1.0 + nu))
