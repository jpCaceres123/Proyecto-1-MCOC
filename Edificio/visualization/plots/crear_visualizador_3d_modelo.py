...
OUTPUT = ROOT / "visualization" / "plots" / "visualizador_modelo_3d.html"
...
<body>
  ...
  <script>
    ...
    function showInfo(slab) {
      ...
      const loadRows = slab.tributary_loads.map(l => `<tr><td>${l.edge}</td><td>${l.beam_ids.join(',')}</td><td>${l.tributary_area_m2.toFixed(2)}</td><td>${l.distribution}</td><td>${l.w_start_kN_m.toFixed(2)}/${l.w_max_kN_m.toFixed(2)}/${l.w_end_kN_m.toFixed(2)}</td></tr>`).join('');
      info.innerHTML = `<b>Losa ${slab.id}</b><br>Z=${slab.z_m.toFixed(2)} m<br>Area=${slab.area_m2.toFixed(2)} m²<br>Dims=${slab.dimensions_m.lx.toFixed(2)} x ${slab.dimensions_m.ly.toFixed(2)} m<br>Nodos=${slab.node_ids.join(', ')}<table><thead><tr><th>Borde</th><th>Viga</th><th>Atrib</th><th>Tipo</th><th>w</th></tr></thead><tbody>${loadRows}</tbody></table>`;
    }
