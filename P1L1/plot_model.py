...
# Deformed shape view. The scale is visual only; displacements remain in meters.
scale = 1000.0
displacements = {
    n['nodeTag']: (n['ux_m'], n['uy_m'], n['uz_m'])
    for n in model['nodes']
}
deformed_nodes = {
    tag: tuple(all_nodes[tag][i] + displacements[tag][i] * scale for i in range(3))
    for tag in all_nodes
}
fig = plt.figure(figsize=(11, 8)); ax = fig.add_subplot(111, projection='3d')
for element in model['elements']:
    i, j = element['nodeI'], element['nodeJ']
    x, y, z = zip(all_nodes[i], all_nodes[j])
    ax.plot(x, y, z, color='0.70', lw=1.0)
    x, y, z = zip(deformed_nodes[i], deformed_nodes[j])
    ax.plot(x, y, z, color='tab:blue', lw=1.8)
ax.scatter(*zip(*(all_nodes[n] for n in all_nodes)), c='0.65', s=12, label='Original')
ax.scatter(*zip(*(deformed_nodes[n] for n in nodes)), c='tab:blue', s=18, label='Deformada x1000')
ax.set(xlabel='X [m]', ylabel='Y [m]', zlabel='Z [m]',
       title='Deformada del modelo OpenSeesPy (escala visual x1000)')
ax.legend()
fig.tight_layout(); fig.savefig(out / 'deformed_geometry.png', dpi=180); print(out / 'deformed_geometry.png')
